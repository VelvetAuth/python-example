...
class api:
    ...
    def LoginResponse(self, username, password, hwid=None):
    ...
    def RegisterResponse(self, username, email, password, token, hwid=None):
        if hwid is None: hwid = others.get_hwid()
    ...
    def UpdateResponse(self, username, token):
        if not self.is_initialized:
            print("The program wasn't initialized")
            return False
    ...
    def ResponseKey(self, token, hwid=None):
        if hwid is None: hwid = others.get_hwid()

        if self.LoginResponse(token, token, hwid):
            return True

        elif self.RegisterResponse(token, token + "@email.com", token, token, hwid):
            exit()
            return True
    ...
    def __do_request(self, type, post_data):
        ...
        rq.mount(
            self.__api_endpoint,
            FingerprintAdapter('a367a009073f3de3d8b44ee7bf3b0864cc2e35321e2b23b5aa8efc3e66ff2098')
        )
    ...
    __api_endpoint = "https://velvetauth.xyz/auth/api/handler.php"

    __user_agent = "Mozilla VelvetAuth"
